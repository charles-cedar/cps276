...
$elementsArr = [
  ...
  "email"=>["errorMessage"=>"<span style='color: red; margin-left: 15px;'>Email cannot be blank</span>",
    "errorOutput"=>"",
    "type"=>"text",
...
function addData($post){
  ...
  /* IF EVERYTHING WORKS ADD THE DATA HERE TO THE DATABASE HERE USING THE $_POST SUPER GLOBAL ARRAY */
      ...
      $emailSql = "SELECT * from admins where admin_email = :email";

			$emailCheck = $pdo->selectBinded($emailSql, $emailBindings);

      if(count($emailCheck) != 0){

        return getForm("<p>It looks like that is a duplicate email </p>", $elementsArr);
      }
